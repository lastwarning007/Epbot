...
API_TOKEN = '7466760045:AAF1E7sLjcu0VinF9LYIjzzs_Qm45l9lThA'
ADMIN_ID = 6829567767
MAX_RESTARTS = 5
...
def start_bot():
    ...
    return subprocess.Popen(['python', 'python encrypted bot.py'])
    ...
    while True:
        if restart_count >= MAX_RESTARTS:
            ...
            if current_time - last_restart_time < RESTART_PERIOD:
                ...
                await notify_admin(f"тЪая╕П Maximum restart limit reached. Waiting for {int(wait_time)} seconds before retrying.")
        ...
        process = start_bot()
        await notify_admin("LAST_WARNING ЁЯЪА Bot is starting...")

        while process.poll() is None:
            await asyncio.sleep(5)
        
        logging.warning("Bot process terminated. Restarting in 10 seconds...")
        await notify_admin("тЪая╕П The bot has crashed and will be restarted in 10 seconds.")
        restart_count += 1
